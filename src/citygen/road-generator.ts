...
export class RoadGenerator {
	...
	private placeSegment(seg: Segment): boolean {
		// Reject segments that extend outside world bounds
		const half = this.cfg.worldSize / 2;
		if (seg.end.x < -half || seg.end.x > half || seg.end.y < -half || seg.end.y > half) return false;
	...
	private branch(seg: Segment, t: number, queue: QueueEntry[]): void {
		...
		const forwardLen = seg.highway ? this.cfg.highwaySegmentLength : this.cfg.normalSegmentLength;
		queue.push({
			t: t + 1,
			seg: {
				start: seg.end,
